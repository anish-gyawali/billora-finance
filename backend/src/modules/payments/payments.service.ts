...
export class PaymentsService {
  ...
  async create(input: CreatePaymentInput, actorId: string, idempotencyKey?: string): Promise<PaymentResponse> { const payment = await this.repository.create(input, actorId, idempotencyKey); const allocation = await this.repository.resolveAllocation(payment.allocated_to_type, payment.allocated_to_id ?? null); if (payment.allocated_to_type !== "direct" && !allocation) throw new BadRequestError("Allocation target was not found"); await this.audit(actorId, "PAYMENT_CREATED", payment.id, { direction: payment.direction, amount: payment.amount.toString(), allocated_to_type: payment.allocated_to_type, allocated_to_id: payment.allocated_to_id }); return { payment, allocation }; }
